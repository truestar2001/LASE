...
def compute_fbank(wav: torch.Tensor, sample_rate: int = 16000, window_type: str = "hamming") -> torch.Tensor:
    feat = kaldi.fbank(
        wav,
        num_mel_bins=80,
        frame_length=25,
...
def main():
    ...
    parser.add_argument(
        "--hf-repo",
        type=str,
        default="Wespeaker/wespeaker-ecapa-tdnn512-LM",
        help="VoxCeleb2-dev-only ECAPA-TDNN checkpoint repo",
    )
    parser.add_argument(
        "--model-dir",
        type=str,
        default="/shared/NAS_HDD/yoon/lase/pretrained/wespeaker_ecapa_tdnn512_lm",
    )
    ...
    print(f"[INFO] hf repo          : {args.hf_repo}")
    ...
    # VoxCeleb2-dev-only ckpt를 직접 로드
    model = wespeaker.load_model_pt(str(model_dir))
    ...
    for dataset_name, wav_path, rel_path in tqdm(all_items, desc=f"Extracting [{BASELINE_NAME}]"):
        ...
        wav = load_audio_16k_mono(wav_path)                  # [1, T], CPU
        feats = compute_fbank(wav, sample_rate=16000)       # [1, T, 80], CPU
        feats = feats.to(args.device)                       # 핵심 수정
    ...
    summary = {
        ...
        "checkpoint_training_data": "VoxCeleb2 Dev dataset",
