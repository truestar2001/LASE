...
def main():
    ...
    parser.add_argument("--hf-repo", type=str, default="Jenthe/ECAPA2")
    parser.add_argument("--hf-filename", type=str, default="ecapa2.pt")
    ...
    model_file = hf_hub_download(repo_id=args.hf_repo, filename=args.hf_filename)
    model = torch.jit.load(model_file, map_location=args.device)
    model.eval()
    ...
    for dataset_name, wav_path, rel_path in tqdm(all_items, desc=f"Extracting [{BASELINE_NAME}]"):
        ...
        wav = load_audio_16k_mono(wav_path).to(args.device)  # [1, T]
